...
async function processPage(pagePath) {
    ...
    if (navElement) {
        ...
        links.forEach(link => {
            ...
            if (href) {
                if (href.includes(pageName) || (targetPath && href.includes(targetPath))) {
...
<div class='muted photo-metadata'>
    ...
    if (targetPath === 'photo') {
        ...
        const linkHtml = '<a href="" onclick="window.scrollTo({top: 0, behavior: \'smooth\'}); return false;">&#x2227;</a>'
...
<div id='rolls_navigation'>
    <!--${rolls_nav_first}-->
    ${rolls_nav_previous}
    <span id='rolls-nav-select' onclick="selectOpen(event)">[&#x2022;&#x2022;&#x2022;<span class="dropdown-icon">&#x2228;</span>]${select_rolls_string}</span>
    ${rolls_nav_next}
    <!--${rolls_nav_last}-->
</div>
    `
